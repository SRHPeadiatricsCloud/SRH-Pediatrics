...
service-worker and Feeds-tab work that reached `main` through PRs #15–#18.

### Added

- **Service worker that cannot pin a stale UI** (`public/sw.js`, `src/components/pwa.tsx`, `vercel.json`,
  `src/app/manifest.ts`): router/RSC payloads are network-only, documents are network-first, the worker
  registers with `updateViaCache: "none"`, `/sw.js` is served with `max-age=0`, and every install clears
  caches that are not `srh-nicu-v3.13.0`.
- **Production UI verification workflow** (`.github/workflows/prod-ui-verify.yml`, dispatch-only) that
  checks the live deployment serves the 3.13.0 worker, the `?v=3.13.0` assets and a bundle containing the
  Feeds tab and its **"+ Advance feeds"** button.
- Feed-interval presets (Q-hours) in `src/lib/catalog.ts`; stale-UI troubleshooting in
  `DEPLOYMENT-SUPABASE-VERCEL.md`.

### Changed

- Version markers (`APP_VERSION`, worker cache name, icon/logo `?v=` query) all move to **3.13.0**
  (chosen over 3.1.0, which reads as older than 3.10.0 / 3.12.2).
- Overview and Daily-progress fortifier summaries now read the merged nutrition model
  (`fortified`, `fortProduct`, `fortKcal`, `fortProtein`) instead of the pre-merge `fortStatus` fields.

### Kept from PR #14

- `src/components/fluids-tab.tsx` (1-tap **+ Advance feeds** with tolerance gate, feed plan, unit protocol,
  formulation reference), `src/lib/feed-guide.ts`, `src/lib/daily-round.ts`, `src/lib/discharge.ts`,
  the BP centiles, this CHANGELOG and everything else released in 3.2.0 – 3.12.2.
