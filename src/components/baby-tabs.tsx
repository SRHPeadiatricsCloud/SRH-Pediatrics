...
type FluidExtras = {
  ...
  fortifiers?: { id: string; name: string; kcalPerUnit: number; referenceVolumeMl: number; phase: number; amount?: number; amountUnit?: "sachet" | "ml" | "unit"; confirmed?: boolean }[];
...
export function FluidsTab({ d, patch, user = "", role = "" }: { d: Detail; patch: (b: Record<string, unknown>) => Promise<unknown>; user?: string; role?: string }) {
  ...
  const fortifierValidationWarnings = fortifiers.filter((fortifier) => !fortifier.name.trim() || !Number.isFinite(fortifier.kcalPerUnit) || fortifier.kcalPerUnit < 0 || !Number.isFinite(fortifier.referenceVolumeMl) || fortifier.referenceVolumeMl <= 0 || !Number.isFinite(fortifier.phase) || fortifier.phase < 0 || fortifier.phase > 1 || !Number.isFinite(fortifier.amount ?? 1) || (fortifier.amount ?? 1) < 0).map((fortifier) => `${fortifier.name || "Unnamed fortifier"}: use kcal ≥ 0, amount ≥ 0, reference volume > 0 ml, and phase 0–1.`);
  ...
  const requestAdvance = () => { if (canApplyAdvance) setAdvanceConfirming(true); };
        ...
        {dailyMode ? (
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            ...
            <div className="md:col-span-2 rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-3">
              ...
              {advanceConfirming && <div className="mt-2 rounded-xl border border-cyan-300/40 bg-cyan-400/10 p-2 text-[11px] text-cyan-50"><b>Confirm this clinical change</b><p className="mt-1">{advanceSection === "enteral" ? "Enteral" : "IV / TPN"}: {showFluid(advanceCurrent)} → {showFluid(advanceNext)} ml/kg/day ({showFluid(advanceCurrent * activeWeightKg)} → {showFluid(advanceNext * activeWeightKg)} ml/day). Dosing weight: {showFluid(activeWeightKg)} kg ({showFluid(activeWeightKg * 1000)} g). Effective: {new Date().toLocaleString([], { hour: "2-digit", minute: "2-digit" })} onward.</p><div className="mt-2 flex gap-2"><button type="button" className="btn-secondary min-h-10" disabled={advanceApplying} onClick={() => setAdvanceConfirming(false)}>Cancel</button><button type="button" className="btn-primary min-h-10" disabled={!canApplyAdvance} onClick={applyAdvance}>{advanceApplying ? "Applying…" : "Confirm & apply"}</button></div></div>}
            </div>
