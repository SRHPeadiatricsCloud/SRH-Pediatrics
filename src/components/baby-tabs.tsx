...
type FluidExtras = {
  ...
  fortifiers?: { id: string; name: string; type?: string; kcalPerUnit: number; proteinPerUnit?: number; referenceVolumeMl: number; phase: number; amount?: number; amountUnit?: "sachet" | "ml" | "g" | "scoop" | "unit"; minimumAmount?: number; maximumAmount?: number; notes?: string; confirmed?: boolean }[];
...
export function FluidsTab({ d, patch, user = "", role = "" }: { d: Detail; patch: (b: Record<string, unknown>) => Promise<unknown>; user?: string; role?: string }) {
  ...
  const fortifierSummary = fortifiers.length ? `${showFluid(idealFeedVolume)} ml ${s.feedType || "milk"} + ${fortifiers.map((fortifier) => `${fortifier.name} ${fortifierPhaseLabel(fortifier.phase)}${fortifierAmountLabel(fortifier)}`).join(" + ")}` : `${showFluid(idealFeedVolume)} ml ${s.feedType || "milk"}`;
  ...
  const save = () => {
    if (!canModify || !dosingWeightConfirmed || mismatch || planLimitError || volumeInvalid || conflict || hasFortifierValidationError || fortifierReferencePending) return;
  ...
  const planStatus: "Draft" | "Active" | "Warning" | "Blocked" = conflict || mismatch || volumeInvalid || planLimitError || hasFortifierValidationError || fortifierReferencePending ? "Blocked" : !dosingWeightConfirmed ? "Draft" : warnings.length ? "Warning" : "Active";
  ...
  return (
    <div className="grid gap-3">
      <Section
        ...
        right={
          <div className="flex items-center gap-2">
            <span className={`rounded-full px-2 py-1 text-[9px] font-black ${planStatusClass}`}>Plan: {planStatus}</span><span className="hidden text-[10px] text-emerald-300 sm:inline">Auto-save on</span>
            <button className="btn-primary" disabled={!canModify || !dosingWeightConfirmed || mismatch || planLimitError || volumeInvalid || !!conflict || hasFortifierValidationError || fortifierReferencePending} onClick={save}>{mismatch ? "Reconcile before saving" : planLimitError || volumeInvalid ? "Fix fluid validation" : !dosingWeightConfirmed ? "Confirm dosing weight" : hasFortifierValidationError ? "Fix composition" : fortifierReferencePending ? "Confirm fortifier amount" : "Save 24-hour plan"}</button>
          </div>
        }
      >
        <div className="mb-2 flex flex-wrap items-center gap-x-3 gap-y-1 rounded-xl border border-white/10 bg-slate-900/35 px-3 py-2 text-[10px] text-slate-400"><b className="text-slate-100">{d.baby.babyName}</b><span>Identifier {d.baby.uhid || `#${d.baby.id}`}</span><span>GA {d.baby.gestWeeks}+{d.baby.gestDays}</span><span>Postnatal day {day}</span><span>Current {showFluid(d.baby.currentWeight / 1000)} kg</span><span>Updated {fmtTime(d.baby.updatedAt)}</span></div>
        ...
        {dailyMode ? (
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/35 p-2">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                ...
                <NumField label="Maximum" value={s.plan?.iv?.maximumMlKgDay} onChange={setPlanNumber("iv", "maximumMlKgDay")} min={0} max={300} step={1} disabled={!canModify} placeholder="160" />
              </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-4">
            <div><NumField label="Total fluids ml/kg/d" value={s.totalMlKgDay ?? undefined} onChange={(n) => updateFixedFluid("total", n)} min={0} max={300} step={1} disabled={!canModify} placeholder="enter" /><span className="ml-1 text-[9px] text-slate-500">{fluidDriver === "total" ? "● Driving · enteral + IV follow" : "Following / derived"} · {activeWeightKg ? `= ${showFluid(currentTotal * activeWeightKg)} ml/day` : "confirm kg"}</span></div>
            <div><NumField label="Enteral ml/kg/d" value={s.enteralMlKgDay ?? undefined} onChange={(n) => updateFixedFluid("enteral", n)} min={0} max={300} step={1} disabled={!canModify} placeholder="enter" /><span className="ml-1 text-[9px] text-slate-500">{fluidDriver === "enteral" ? "● Driving · IV follows" : "Following / derived"} · {activeWeightKg ? `= ${showFluid(currentEnteral * activeWeightKg)} ml/day` : "confirm kg"}</span></div>
            <div><NumField label="IV ml/kg/d" value={s.ivMlKgDay ?? undefined} onChange={(n) => updateFixedFluid("iv", n)} min={0} max={300} step={1} disabled={!canModify} placeholder="enter" /><span className="ml-1 text-[9px] text-slate-500">{fluidDriver === "iv" ? "● Driving · enteral follows" : "Following / derived"} · {activeWeightKg ? `= ${showFluid(currentIv * activeWeightKg)} ml/day` : "confirm kg"}</span></div>
            ...
            <div><NumField label="Amino acid g/kg/d" value={s.aminoAcid ?? undefined} onChange={set("aminoAcid")} min={0} max={5} step={0.1} decimals={1} disabled={!canModify} placeholder="enter" /><span className="ml-1 text-[9px] text-slate-500">{activeWeightKg && s.aminoAcid != null ? `= ${showFluid(s.aminoAcid * activeWeightKg)} g/day` : "confirm kg for g/day"}</span></div>
            <div><NumField label="Lipid g/kg/d" value={s.lipid ?? undefined} onChange={set("lipid")} min={0} max={5} step={0.1} decimals={1} disabled={!canModify} placeholder="enter" /><span className="ml-1 text-[9px] text-slate-500">{activeWeightKg && s.lipid != null ? `= ${showFluid(s.lipid * activeWeightKg)} g/day` : "confirm kg for g/day"}</span></div>
            <div><NumField label="Energy target kcal/kg/d" value={s.kcal ?? undefined} onChange={set("kcal")} min={0} max={200} step={1} disabled={!canModify} placeholder="enter" /><span className="ml-1 text-[9px] text-slate-500">{activeWeightKg && s.kcal != null ? `= ${showFluid(s.kcal * activeWeightKg)} kcal/day` : "confirm kg for kcal/day"}</span></div>
        ...
        <div className="mt-3 rounded-xl border border-white/10 bg-slate-900/35 p-3">
          ...
          {(nutrition.mctKcal > 0 || nutrition.fortifierKcal > 0 || nutrition.fortifierProtein > 0) && <div className="mt-2 flex flex-wrap gap-2 text-[10px] text-slate-300"><span className="rounded-lg bg-white/5 px-2 py-1">Fortifier calories: {showFluid(nutrition.fortifierKcal)} kcal/kg/day · {activeWeightKg ? `${showFluid(nutrition.fortifierKcal * activeWeightKg)} kcal/day` : "confirm kg"}</span>{nutrition.mctKcal > 0 && <FormulaValue onOpen={() => setFormula({ title: "MCT contribution", text: `MCT contribution = enteral ml/kg/day × configured MCT kcal/ml contribution = ${showFluid(nutrition.mctKcal)} kcal/kg/day.` })} className="rounded-lg bg-amber-400/10 px-2 py-1 text-amber-100">MCT: {showFluid(nutrition.mctKcal)} kcal/kg/day{activeWeightKg ? ` · ${showFluid(nutrition.mctKcal * activeWeightKg)} kcal/day` : ""}</FormulaValue>}</div>}
        </div>
        ...
        <div className="mt-3 rounded-xl border border-white/10 bg-slate-900/35">
          ...
          {configOpen && <div className="space-y-3 border-t border-white/10 p-2">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-950/20 p-2"><div className="mb-2 flex items-center justify-between"><b className="text-xs text-slate-100">Authorized target settings</b><span className="text-[9px] text-slate-500">changes are audited</span></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-5"><NumField label="Energy low" value={s.targets?.energy?.[0]} onChange={(value) => updateTarget("energy", 0, value)} min={0} max={300} step={1} disabled={!canModify} placeholder="110" /><NumField label="Energy high" value={s.targets?.energy?.[1]} onChange={(value) => updateTarget("energy", 1, value)} min={0} max={300} step={1} disabled={!canModify} placeholder="135" /><NumField label="GIR low" value={s.targets?.gir?.[0]} onChange={(value) => updateTarget("gir", 0, value)} min={0} max={30} step={0.1} disabled={!canModify} placeholder="4" /><NumField label="GIR high" value={s.targets?.gir?.[1]} onChange={(value) => updateTarget("gir", 1, value)} min={0} max={30} step={0.1} disabled={!canModify} placeholder="12" /><NumField label="Max enteral" value={s.targets?.maximumEnteralMlKgDay} onChange={(value) => { if (canModify) setS((p) => ({ ...p, targets: { ...(p.targets ?? {}), maximumEnteralMlKgDay: value } })); }} min={0} max={300} step={1} disabled={!canModify} placeholder="150" /></div></div>
          </div>}
        </div>
