  ...
  if (moduleView === "today") {
    ...
    return (
      <div className="grid gap-3">
        <Section title="Fluids, TPN & nutrition · Today’s plan" sub="One card for the whole bedside read-back. Expand details only when needed." right={<div className="flex flex-wrap gap-2"><button type="button" className="btn-secondary min-h-10" onClick={sharePlan} disabled={!planExists}>Share plan</button><button type="button" className="btn-ghost min-h-10" onClick={() => rememberView("advanced")}>Advanced view</button><button type="button" className="btn-primary min-h-10" disabled={!canModify} onClick={startWizard}>{planExists ? "Update plan" : "Start plan"}</button></div>}>
          {header}
