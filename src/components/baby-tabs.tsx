...
import { CalendarClock, CheckCircle2, ChevronDown, Circle, Clock, Info, User, X } from "lucide-react";
...
export function CourseTab({
  ...
  // Targets sized to the age the baby actually reached, not to today.
  const dolAtCourse = Math.max(1, Math.round((+new Date(dischargeDate) - +new Date(b.dob)) / 86400000) + 1);
  const n = calcNutrition(c, lastWeight, { dol: dolAtCourse });
  const course = [
