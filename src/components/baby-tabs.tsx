...
export function FluidsTab({ d, patch, user = "", role = "" }: { d: Detail; patch: (b: Record<string, unknown>) => Promise<unknown>; user?: string; role?: string }) {
  ...
  const planLimitError = (dailyMode && ((s.plan?.enteral?.minimumMlKgDay ?? 0) > (s.plan?.enteral?.maximumMlKgDay ?? 300) || (s.plan?.iv?.minimumMlKgDay ?? 0) > (s.plan?.iv?.maximumMlKgDay ?? 300))) || Boolean(s.fluidRestricted && s.restrictedFluidRange && (s.restrictedFluidRange[0] > s.restrictedFluidRange[1] || s.restrictedFluidRange[0] < 0));
  ...
  const standardFluidRange = s.targets?.fluids ?? neonatalDayFluidRange(d.baby);
  const fluidRange = s.fluidRestricted ? (s.restrictedFluidRange ?? standardFluidRange) : standardFluidRange;
  ...
  if (planLimitError) warnings.push(s.fluidRestricted && s.restrictedFluidRange && s.restrictedFluidRange[0] > s.restrictedFluidRange[1] ? "Fluid-restricted low target cannot exceed its high target." : "Plan minimum cannot exceed its maximum — correct the limits before applying.");
      ...
      >
        ...
        <div className="mt-3 rounded-xl border border-white/10 bg-slate-900/35">
          ...
          {configOpen && <div className="space-y-3 border-t border-white/10 p-2">
            <div className="rounded-xl border border-amber-300/25 bg-amber-400/5 p-2"><div className="flex flex-wrap items-center justify-between gap-2"><div><b className="text-xs text-amber-100">Fluid restriction</b><p className="text-[10px] text-slate-400">Credentialed roles only · changes are confirmed and audited.</p></div><label className="flex min-h-10 items-center gap-2 text-xs font-bold text-amber-100"><input type="checkbox" className="accent-amber-400" checked={Boolean(s.fluidRestricted)} disabled={!canModify} onChange={(event) => { setS((p) => ({ ...p, fluidRestricted: event.target.checked })); setTargetChangePending("Fluid restriction flag changed. Confirm this target change before it is saved."); }} /> Fluid restricted</label></div>{s.fluidRestricted && <div className="mt-2 grid grid-cols-2 gap-2"><NumField strict label="Restricted low ml/kg/day" value={s.restrictedFluidRange?.[0] ?? standardFluidRange[0]} onChange={(value) => { setS((p) => ({ ...p, restrictedFluidRange: [value, p.restrictedFluidRange?.[1] ?? standardFluidRange[1]] })); setTargetChangePending("Restricted fluid low target changed. Confirm this target change before it is saved."); }} min={0} max={300} step={1} disabled={!canModify} /><NumField strict label="Restricted high ml/kg/day" value={s.restrictedFluidRange?.[1] ?? standardFluidRange[1]} onChange={(value) => { setS((p) => ({ ...p, restrictedFluidRange: [p.restrictedFluidRange?.[0] ?? standardFluidRange[0], value] })); setTargetChangePending("Restricted fluid high target changed. Confirm this target change before it is saved."); }} min={0} max={300} step={1} disabled={!canModify} /></div>}</div>
