...
export function PediatricDrugDoses() {
  ...
  return <div className="dose-workspace dose-simple">
    <section className="dose-hero dose-simple-hero"><div><span className="dose-kicker">Paediatric dose navigator · chart 2025</span><h1>Drugs &amp; doses</h1></div><div className="dose-hero-badge"><ShieldAlert size={15} /><span><b>Verify before giving</b><small>Indication · age · maximum · concentration</small></span></div></section>

    <section className="dose-simple-card dose-simple-medicine"><div className="dose-simple-heading"><div><span>1 · Medicine</span><h2>What are you giving?</h2></div><b className="dose-simple-count">{PEDIATRIC_DRUGS.length}</b></div><div className="dose-simple-medicine-controls"><label className="dose-select-field"><span>Category</span><select value={group} onChange={(event) => chooseGroup(event.target.value as PediatricDrugGroup)}>{PEDIATRIC_DRUG_GROUPS.map((item) => <option value={item} key={item}>{item} · {PEDIATRIC_DRUGS.filter((drugItem) => drugItem.group === item).length}</option>)}</select></label><label className="dose-search-field"><span>Search medicine</span><div><Search size={16} aria-hidden="true" /><input type="search" value={drugQuery} onChange={(event) => setDrugQuery(event.target.value)} placeholder="Type a medicine name" aria-label="Search paediatric medicines" /></div></label></div><label className="dose-select-field dose-simple-medicine-select"><span>Medicine</span><select value={drugId} onChange={(event) => chooseDrug(event.target.value)}>{medicineOptions.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.regimens.length} chart row{item.regimens.length === 1 ? "" : "s"}</option>)}</select><small>{matches.length} medicine{matches.length === 1 ? "" : "s"} in this category match the search.</small></label><div className="dose-simple-selected"><span>{drug.name.slice(0, 1)}</span><div><small>Selected medicine</small><b>{drug.name}</b></div><Check size={16} /></div></section>

    <section className="dose-simple-card dose-simple-calculator"><div className="dose-simple-heading"><div><span>2 · Calculator</span><h2>{drug.name}</h2></div><span className={result ? "dose-status ready" : "dose-status"}>{result ? "Ready to review" : "Enter details"}</span></div><div className="dose-simple-row"><label className="dose-select-field"><span>Clinical indication / age row</span><select value={regimenIndex} onChange={(event) => setRegimenIndex(Number(event.target.value))}>{drug.regimens.map((item, index) => <option value={index} key={`${item.condition}-${index}`}>{item.condition} — {item.regimen}</option>)}</select></label><div className="dose-simple-row-preview"><small>Selected chart regimen</small><b>{selectedRow.regimen}</b><span>{selectedRow.rule ? "Calculable" : "Manual verification"}</span></div></div><div className="dose-simple-input-heading"><b>Patient details</b><small>Tap, type, or use ±.</small></div><div className="dose-simple-slider-grid"><Stepper label="Weight" value={weightKg} onChange={setWeightKg} min={0.4} max={200} step={0.1} unit="kg" decimals={1} /><Stepper label="Age" value={ageYears} onChange={setAgeYears} min={0} max={18} step={0.1} unit="years" decimals={1} /><Stepper label="Height" value={heightCm} onChange={setHeightCm} min={30} max={220} step={0.5} unit="cm" decimals={1} /></div><label className="dose-strength-field dose-simple-strength"><span>Product strength <em>optional · calculates volume</em></span><div><input type="number" inputMode="decimal" min={0} step="any" value={strength} onChange={(event) => setStrength(event.target.value)} placeholder="Read exact strength from label" /><b>{drug.strengthUnit ?? "product-specific"}</b></div><small>From the product label</small></label><button type="button" className="dose-clear dose-simple-clear" onClick={clearPatient}>Clear patient details</button></section>

    <section className={`dose-simple-result ${result ? "ready" : ""}`}><div className="dose-simple-result-heading"><div><span>3 · Live result</span><h2>{result ? "Review before administration" : "Your calculated dose appears here"}</h2></div><b>{result ? <><Check size={13} /> Ready</> : "Waiting"}</b></div>{result ? <><div className="dose-simple-result-main"><small>Patient-specific amount</small><strong>{result.calculatedAmount ?? result.dose}</strong><span>{result.dose}</span></div><div className="dose-simple-result-facts"><div><small>How often</small><b>{result.frequency}</b></div><div><small>Route</small><b>{result.route ?? "Per protocol"}</b></div>{result.volume && <div><small>Draw up</small><b>{result.volume}</b></div>}</div></> : <div className="dose-simple-placeholder"><Info size={18} /><p>Enter weight and age.</p></div>}<div className="dose-simple-safety"><ShieldAlert size={14} /><span>{result?.caution ?? "Verify indication, age band, renal/hepatic function, maximum dose and product concentration."}</span></div></section>
